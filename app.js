...
mongoose.connect("mongodb+srv://ID:PASSWORD@cluster0.9gswc.mongodb.net/namelDB?retryWrites=true&w=majority", {useNewUrlParser: true, useUnifiedTopology: true})
...
app.get("/post/:topic", function(req, res) {
    ...
    const title = req.body.textValue

    Post.findOne({title: topic}, function(err, foundPost){
      if(_.lowerCase(foundPost.title) === _.lowerCase(topic)) {
        res.render("post", {
          postTitle: foundPost.title,
          postContant: foundPost.content
        })
      } else {
        console.log(p.title);
        console.log(topic)
